...
class PHP_CRUD_API
{
        ...
        if (is_callable($callback, true)) {
            foreach ($fields as $table => $keys) {
                foreach ($keys as $field) {
                    ...
                    if ($v !== null) {
                        if (is_array($v)) {
                            $this->addFilter($filters, $table, 'and', $field->name, 'in', implode(',', $v));
                        } else {
                            $this->addFilter($filters, $table, 'and', $field->name, 'eq', $v);
                        }
    ...
    protected function findRelations($tables, $database, $auto_include)
    {
        ...
        while (count($tables) > 1) {
            ...
            while ($row = $this->db->fetchRow($result)) {
                if (!$auto_include && !in_array($row[0], array_merge($tables, $tableset))) {
                    continue;
                }
            ...
            while ($row = $this->db->fetchRow($result)) {
                if (!$auto_include && !in_array($row[2], array_merge($tables, $tableset))) {
                    continue;
                }
            ...
            while ($row = $this->db->fetchRow($result)) {
                ...
                if (!$auto_include && !in_array($row[4], array_merge($tables, $tableset))) {
                    continue;
                }
    ...
    protected function readCommand($parameters)
    {
        extract($parameters);
        if (count($key[0]) > 1) {
            $object = $this->retrieveObjects($key, $fields, $filters, $tables);
        } else {
    ...
    protected function createCommand($parameters)
    {
        ...
        if (count($inputs) > 1) {
            return $this->createObjects($inputs, $tables);
        }
    ...
    protected function updateCommand($parameters)
    {
        ...
        if (count($inputs) > 1) {
            return $this->updateObjects($key, $inputs, $filters, $tables);
        }
    ...
    protected function deleteCommand($parameters)
    {
        extract($parameters);
        if (count($key[0]) > 1) {
            return $this->deleteObjects($key, $filters, $tables);
        }
    ...
    protected function incrementCommand($parameters)
    {
        ...
        if (count($inputs) > 1) {
            return $this->incrementObjects($key, $inputs, $filters, $tables, $fields);
        }
        ...
        if ($_FILES) {
            ...
            foreach ($_FILES as $name => $file) {
                foreach ($file as $key => $value) {
                    switch ($key) {
                        case 'tmp_name':$files[$name] = $value ? base64_encode(file_get_contents($value)) : '';
                            break;
            ...
            while ($row = $this->db->fetchRow($result)) {
                ...
                if ($this->db->isNumericType($table_fields[$table['name']][$row[0]])) {
                    if (strpos(strtolower($table_fields[$table['name']][$row[0]]->{'x-dbtype'}), 'int') !== false) {
                        ...
                        if ($row[1] !== null) {
                            $table_fields[$table['name']][$row[0]]->default = (int) $row[1];
                        }

                    } else {
                        $table_fields[$table['name']][$row[0]]->type = 'number';
                        if ($row[1] !== null) {
                            $table_fields[$table['name']][$row[0]]->default = (float) $row[1];
                        }
                    ...
                    if ($row[1] !== null) {
                        $table_fields[$table['name']][$row[0]]->default = $row[1];
                    }

                    if ($row[4] !== null) {
                        $table_fields[$table['name']][$row[0]]->maxLength = (int) $row[4];
                    }
    ...
    public function executeCommand()
    {
        ...
        if (!$this->settings['request']) {
            ...
            switch ($parameters['action']) {
                case 'list':$output = $this->listCommand($parameters);
                    break;
                case 'read':$output = $this->readCommand($parameters);
                    break;
                case 'create':$output = $this->createCommand($parameters);
                    break;
                case 'update':$output = $this->updateCommand($parameters);
                    break;
                case 'delete':$output = $this->deleteCommand($parameters);
                    break;
                case 'increment':$output = $this->incrementCommand($parameters);
                    break;
                case 'headers':$output = $this->headersCommand($parameters);
                    break;
